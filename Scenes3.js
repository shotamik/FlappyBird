...
let XVELOCITYEasy = 40;





class Scenes3 extends Phaser.Scene{
    constructor(){
        super('PlayGame');
    }
    ...
    create(){
        ...
        this.wallPool = [];
        for(let i = 0; i < WALLCOUNTEREASY; i++){
        ...
        this.wallGroup.setVelocityX(-gameOptions.playerSpeed - 30);
    ...
    placeWalls(addScore){
        ...
        let wallHoleHeight = Phaser.Math.Between(gameOptions.wallHoleEasygame[0], gameOptions.wallHoleEasygame[1]);
    ...
    flap(){
        ...
        this.player.body.velocity.x = XVELOCITYEasy;
    ...
    update(){
        ...
        this.wallGroup.getChildren().forEach(function(wall){
            ...
            if(WALLCOUNTEREASY == 7){
                let columnPos = WALLCOUNTEREASY * -190;
            ...
            if(WALLCOUNTEREASY == 8){
                let columnPos = WALLCOUNTEREASY * -195;
                if(wall.getBounds().right < columnPos){
                    this.gameWon();  
            ...
            if(WALLCOUNTEREASY == 9){
                let columnPos = WALLCOUNTEREASY * -196;
                if(wall.getBounds().right < columnPos){
            ...
            if(WALLCOUNTEREASY == 10){
                let columnPos = WALLCOUNTEREASY * -200;
                if(wall.getBounds().right < columnPos){
    ...
    notEnough(){
        if(CREDITS === 0){
            ...
            this.add.text(config.width / 2 - 100, config.height / 2, "Not Enough Credits", style).setDepth(5);
            this.scene.pause();
        }
    }
    ...
    obstaclesQuantity (){
        let style = { font: "bold 32px Arial", fill: "#fff" }
        this.add.text(config.width / 2 + 290, 60, `Obstacles: ${WALLCOUNTEREASY}`, style).setDepth(4);
